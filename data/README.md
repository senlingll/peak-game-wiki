...
`date` must match the UTC build date and a published source must have an `http` or `https` URL. A record becomes a verified entry only when at least two of the map/route/biome values are present. A dated media-only community post may still show its source image/video while those location fields remain pending. `resetAt` is optional only when the reset time cannot be verified. Supported media types are `image` and `video`; media is never used to infer a route. The Steam fallback is accepted only when its current cycle and explicit biome fields are present in HTML; the fetcher does not OCR images or infer names from an unlabeled screenshot.
