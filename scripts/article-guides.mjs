...
export const articleGuides = {
  ...
  'what-is-peak-game': {
    ...
    related: [
      ...
      ['peak-game-price', 'Compare PEAK game prices by region'],
    ],
  },
  ...
  'peak-cooking-guide': {
    slug: 'peak-cooking-guide',
    meta: {
      title: 'PEAK Cooking Guide: Stove Steps & Item Effects',
      description: 'Learn how to cook items in PEAK, compare 16 documented food effects, and check risky non-food interactions plus the Patch 2.4.C amulet rule.',
      schema: 'A source-dated PEAK cooking guide covering the Portable Stove, food cooking effects, non-food item interactions, patch context, and common questions.',
    },
    published: '2026-09-25',
    primaryKeyword: 'PEAK cooking',
    eyebrow: 'Field guide - source-dated item effects',
    h1: 'PEAK Cooking Guide',
    intro: 'This PEAK cooking guide explains what the Portable Stove is documented to do, which food items have a listed cooked effect, and why cooking every object is not a safe default. Start with the item-specific tables, then check the current game prompt before spending a stove. For the wider item catalog, use the [[link:items|PEAK Items database]].',
    answerLabel: 'Short answer',
    answer: 'The community-maintained PEAK Wiki Items table says the Portable Stove is used to cook items and lasts 60 seconds after placement. Cooking is not a universal buff: 16 food entries in the inspected snapshot have a specific cooking note, while non-food results include no effect, incineration, and damaging explosions. That item snapshot was last edited on 2026-08-14. A newer official Steam update, Patch 2.4.C on 2026-09-14, says Amulets can no longer be cooked. Use that patch note for Amulets and treat older item rows as a dated reference, not live telemetry.',
    tocLabel: 'On this page',
    tocFaq: 'Cooking FAQ',
    heroImage: {
      src: '/media/items/portable-stove.webp',
      presentation: 'item-icon',
      alt: 'Portable Stove item image from the community-maintained PEAK Wiki, identifying the cooking tool',
      caption: 'Community-maintained PEAK Wiki item image; it identifies the tool and is not an official gameplay screenshot.',
      width: 128,
      height: 128,
    },
    sections: [
      {
        id: 'how-to-cook-items',
        title: 'How to cook items in PEAK without guessing',
        paragraphs: [
          'The maintained item table identifies the Portable Stove as a tool used to cook items and says it lasts 60 seconds after placement. The source does not document a reliable button-by-button recipe for every control layout, so follow the prompt shown by your current game instead of relying on an old keybind. The practical first step is to identify the exact item name and read its cooking note before placing the stove.',
          'A cooked result can be very different from the raw item. Some food entries describe removal of a status effect; some non-food entries report an explosion or immediate incineration. Do not assume that an item is safe to cook because it can be picked up, consumed, or carried. The tables below keep food and non-food behavior separate so a helpful status-cleanse does not get confused with a hazard.',
          'Check the game version when a result matters. The item-table snapshot used here is dated 2026-08-14, while official Patch 2.4.C was posted on 2026-09-14 and added the specific rule that Amulets can no longer be cooked. If a current in-game prompt or later official note differs from an older row, use the newer evidence and record the version rather than repeating a stale tip.',
        ],
        bullets: [
          ['Identify the item', 'Use the canonical item name so a food effect is not confused with a similarly named tool.'],
          ['Read the cooked note', 'Look for an exact effect, a no-effect note, an explosion, or incineration.'],
          ['Check your patch', 'The September 2026 Amulet rule is newer than the item-table snapshot.'],
          ['Follow the live prompt', 'The source does not verify a universal input sequence for every control setup.'],
        ],
      },
      {
        id: 'food-cooking-effects',
        title: 'PEAK cooking effects: documented food items',
        paragraphs: [
          'The table lists the 16 food entries whose inspected item notes contain a cooking result. It is a reference table, not a ranking: a status-removal effect is useful only when that status is relevant, and a note about one cooked item does not prove that every food gains the same benefit. Canonical English item names are retained because the source table does not provide a verified row-by-row localized naming map.',
          'Several rows repeat a narrow result, such as removing Poison or Spores. Fortified Milk, Mandrake, Medicinal Root, and The Early Worm have different notes. Read the item-specific outcome before choosing what to cook, and check the current game after an update if a precise result changes the route decision.',
        ],
        image: {
          src: '/media/items/green-shroomberry.webp',
          presentation: 'item-icon',
          alt: 'Green Shroomberry item image used to identify a food with a documented cooked Spores effect',
          caption: 'Community-maintained PEAK Wiki item image; the table states the documented cooked effect separately.',
          width: 128,
          height: 128,
        },
        table: {
          caption: 'Food cooking notes in the inspected PEAK Wiki Items snapshot, last edited 2026-08-14',
          headers: ['Food item', 'Documented cooked result'],
          rows: [
            ['Big Lollipop', 'Removes Drowsy.'],
            ['Blue Shroomberry', 'Removes 5 Spores.'],
            ['Fortified Milk', 'Adds 10 seconds to invulnerability.'],
            ['Gold Prickleberry', 'Removes Poison.'],
            ['Green Crispberry', 'Removes Poison.'],
            ['Green Kingberry', 'Removes Poison.'],
            ['Green Shroomberry', 'Removes 5 Spores.'],
            ['Mandrake', 'Disables its scream and removes 25 Curse.'],
            ['Medicinal Root', 'Removes an extra 20 Poison and 20 Spores.'],
            ['Pink Berrynana', 'Removes Poison.'],
            ['Purple Shroomberry', 'Removes 5 Spores.'],
            ['Red Prickleberry', 'Removes Thorns.'],
            ['Red Shroomberry', 'Removes 5 Spores.'],
            ['The Early Worm', 'Gains no additional bonus and stops moving.'],
            ['Yellow Kingberry', 'Removes Poison.'],
            ['Yellow Shroomberry', 'Removes 5 Spores.'],
          ],
        },
        afterTable: 'The values and wording above are transcribed from a community-maintained item reference. They do not establish whether a later patch changed timing, stacking, raw effects, or the result of cooking the same item more than once.',
      },
      {
        id: 'non-food-interactions',
        title: 'Non-food items can have very different cooking outcomes',
        paragraphs: [
          'PEAK cooking questions often name tools or equipment rather than food. The inspected item table records several interactions, but they are not interchangeable: an Antidote can affect nearby Poison, while a Flare or Portable Stove can injure nearby players, and some gear is immediately incinerated. Keep enough distance from teammates when a listed effect has a 4.8 m radius.',
          'Amulets need a separate version note. The official Patch 2.4.C announcement on 2026-09-14 states “Amulets can no longer be cooked.” That is a current official rule and is newer than the 2026-08-14 item-table snapshot. Do not use an older cooking result to contradict this patch note.',
        ],
        image: {
          src: '/media/items/cooked-bird.webp',
          presentation: 'item-icon',
          alt: 'Cooked Bird item image from the community-maintained PEAK Wiki, shown as a distinct cooking outcome',
          caption: 'Community-maintained PEAK Wiki item image; it is an item reference, not a gameplay capture.',
          width: 128,
          height: 128,
        },
        table: {
          caption: 'Selected non-food cooking interactions and their evidence boundary',
          headers: ['Item', 'Recorded interaction', 'How to use the note'],
          rows: [
            ['Amulets', 'Cannot be cooked under official Patch 2.4.C (2026-09-14).', 'The newer official patch note overrides older assumptions.'],
            ['Antidote', 'Explodes and removes up to 20 Poison from players within 4.8 m.', 'A nearby-player effect; do not treat it as an individual-only action.'],
            ['Anti-Rope Cannon', 'Immediately incinerated.', 'No benefit is recorded in the item note.'],
            ['Bandages', 'No effect.', 'The item row does not record an added cooking result.'],
            ['Flare', 'Explodes and can deal up to 20 Injury within 4.8 m.', 'Keep teammates outside the documented radius.'],
            ['The Book of Bones', 'No effect.', 'The item row explicitly records no cooking effect.'],
            ['Portable Stove', 'Explodes and can deal up to 20 Injury within 4.8 m.', 'The cooker itself is also listed as a cooking hazard.'],
            ['Rescue Claw', 'Immediately incinerated.', 'Do not spend the tool expecting a cooked bonus.'],
            ['Backpack', 'No cooking result is recorded in the inspected snapshot.', 'Absence of a note is not proof of a hidden or future effect.'],
          ],
        },
      },
      {
        id: 'choose-what-to-cook',
        title: 'Choose what to cook for the next climb',
        paragraphs: [
          'Start with the problem the team actually has. If a Scout is poisoned or covered in Spores, compare the specific status-removal rows; if nobody has that status, cooking an item for that effect may not help the next section. A Drowsy or Curse note is similarly situational. This is why a general “best cooked food” tier list would be misleading without a stable route, team state, and current patch.',
          'Next, check whether an effect reaches nearby players. The Antidote, Flare, and Portable Stove notes describe a 4.8 m radius. In co-op, that radius can help or harm more than one Scout. Tell the group what will happen before using a proximity effect, and avoid placing a damaging interaction beside a teammate who is recovering or standing on a narrow ledge.',
          'Finally, compare the cooking result with the original item and the cost of using the stove. The available source snapshot does not establish that cooking the same food twice grants an extra benefit, nor does it provide a complete live list of current drop locations. For item stats, raw effects, and biome notes, continue to the [[link:items|PEAK Items database]]. For the Cryptogastronomy badge, check the existing [[link:badges-guide|PEAK badges guide]] rather than treating every food effect as a badge condition.',
        ],
        bullets: [
          ['Match the effect to a status', 'Do not spend an item on a benefit the team cannot use.'],
          ['Check the radius', 'Explosive cooking notes can include nearby teammates.'],
          ['Do not assume stacking', 'No second-cook bonus is established by the cited snapshot.'],
          ['Keep objectives separate', 'Food effects and badge requirements are different questions.'],
        ],
      },
      {
        id: 'source-and-update-check',
        title: 'What this cooking guide can verify',
        paragraphs: [
          'The food and item interactions above come from the community-maintained PEAK Wiki Items page. The site snapshot identifies that page as last edited on 2026-08-14; this guide preserves that date instead of describing its rows as live or exhaustive. Its recorded 51 food entries include 16 food rows with a cooking note. The page is a useful item reference, but it is not a developer patch log.',
          'For changes after that snapshot, use the official Steam news feed. The latest saved announcement checked for this guide is Patch 2.4.C, dated 2026-09-14, and it explicitly changes the Amulet cooking rule. The announcement does not validate every other row in the older community item table. Recheck a mechanic in the current build when a decision depends on an interaction not named in the patch or table.',
          'This page is an independent fan reference. Community item images are credited through the linked item source and the site-wide CC BY-SA notice; they are not presented as official Steam screenshots. If a later source confirms a changed effect, update the row and its date rather than silently treating a player report as official confirmation.',
        ],
      },
    ],
    faq: {
      eyebrow: 'Common questions',
      title: 'PEAK cooking FAQ',
      items: [
        ['How do you cook items in PEAK?', 'The community-maintained item table says the Portable Stove is used to cook items and lasts 60 seconds after placement. Follow the current in-game prompt; the inspected source does not verify one universal button sequence for every control setup.'],
        ['What does cooking Fortified Milk do in PEAK?', 'The inspected item row says cooking Fortified Milk adds 10 seconds to invulnerability. Treat this as a dated community-table value and recheck it after patches.'],
        ['Does cooking every food in PEAK give a benefit?', 'No universal benefit is documented. The inspected snapshot lists 16 food cooking notes, including The Early Worm gaining no additional bonus; use the item-specific row rather than assuming a shared buff.'],
        ['Should you cook food once or twice in PEAK?', 'The sources checked for this guide do not establish an extra benefit from cooking the same food again. Do not spend another item or stove on a repeat result unless the current game shows a useful effect.'],
        ['What does cooking The Book of Bones do?', 'Its inspected item note says “Cooking: No effect.” The row is from a community-maintained snapshot last edited 2026-08-14, so confirm in-game if a later patch changes it.'],
        ['What does cooking an Antidote do in PEAK?', 'The item note says it explodes and removes up to 20 Poison from all players within 4.8 m. The radius makes teammate position relevant.'],
        ['Can you cook Amulets in PEAK?', 'No. Official Patch 2.4.C, dated 2026-09-14, states that Amulets can no longer be cooked. This newer patch rule takes precedence over older item-table assumptions.'],
        ['What happens when you cook a Flare in PEAK?', 'The inspected item note says the Flare explodes and can deal up to 20 Injury to all players within 4.8 m. Keep the group clear of the radius.'],
        ['Do cooked Bandages or a First Aid Kit have an effect?', 'The inspected item table records “No effect” for both Bandages and the First Aid Kit. These are source-dated entries, not a promise that later builds cannot change them.'],
        ['Does cooking water do anything in PEAK?', 'The item source checked here does not verify a water-specific cooking effect. This guide does not claim that water grants a buff or changes another item.'],
      ],
    },
    source: {
      eyebrow: 'Sources and limits',
      title: 'Use the patch note for current changes',
      body: 'The item rows are transcribed from a community-maintained PEAK Wiki snapshot last edited 2026-08-14. The Amulet rule comes from the official 2026-09-14 Patch 2.4.C announcement. These sources answer different questions; the patch note takes precedence for the changed Amulet mechanic, and the older table is not described as a complete live list.',
      links: [
        ['Official PEAK Steam news feed - Patch 2.4.C', 'https://store.steampowered.com/news/app/3527290'],
        ['Community-maintained PEAK Wiki Items table', 'https://peak.wiki.gg/wiki/Items'],
      ],
    },
    related: [
      ['items', 'Browse the complete PEAK Items database'],
      ['badges-guide', 'Check the PEAK cooking badge guide'],
      ['map-rotation', 'Plan around the current PEAK map'],
    ],
    relatedLabel: 'Related PEAK references',
  },
  'peak-game-price': {
    slug: 'peak-game-price',
    meta: {
      title: 'PEAK Game Price in 2026: Steam Prices by Region',
      description: 'PEAK game price snapshot checked September 25, 2026: compare nine official Steam regions and confirm the live checkout price before buying.',
      schema: 'A dated, source-backed comparison of PEAK Steam list and sale prices in nine regional storefronts, with clear limits and steps for checking the live checkout price.',
    },
    published: '2026-09-25',
    primaryKeyword: 'peak game price',
    eyebrow: 'Dated Steam price snapshot',
    h1: 'PEAK Game Price: Steam Prices by Region',
    intro: 'The PEAK game price depends on the Steam storefront region. In the official price data checked on September 25, 2026, the US listing showed a $7.99 list price and a temporary $4.95 sale price; the same snapshot showed a 38% discount in the other eight markets below. These are dated figures, not a promise that the discount is still active. Compare the regional table, then open the official Steam listing for the price shown at your own checkout.',
    answerLabel: 'The short answer',
    answer: 'At the September 25, 2026 check, PEAK was listed at $7.99 in the US and CNY 33.00 in China before discounts. The official Steam data showed temporary sale prices of $4.95 and CNY 20.46 respectively; all nine checked markets were discounted by 38% at that moment. Steam prices, sale availability, displayed currency, and checkout totals can change. Use this table as a dated comparison and treat the live regional Steam page and checkout as final.',
    tocLabel: 'On this page',
    tocFaq: 'Price FAQ',
    heroImage: {
      src: '/media/peak-final-ascent.webp',
      alt: 'PEAK key art showing scouts approaching the final summit',
      caption: 'Official PEAK store artwork. This identifies the game; it is not a screenshot of a price or discount.',
      width: 1200,
      height: 675,
    },
    sections: [
      {
        id: 'price-by-region',
        title: 'PEAK game price by Steam region',
        paragraphs: [
          'The table records the regular and sale prices returned by Steam’s official app-details storefront data on September 25, 2026. Each row uses the currency reported for that market. The sale values were all 38% below list price in this single check, but Steam did not return an offer end date in the response we reviewed. Do not assume a discount remains available because it appears here.',
          'A regional comparison is useful when you are planning a purchase with friends in different countries or deciding whether the listed amount fits your budget. It does not tell you what another person will pay at checkout: the account’s storefront country, local tax rules, payment method, and current offer can affect the final display. If the live listing differs from this dated table, trust the listing tied to your own Steam account.',
        ],
        table: {
          caption: 'Official Steam PEAK prices checked September 25, 2026',
          headers: ['Steam market', 'List price', 'Sale price at check'],
          rows: [
            ['United States (USD)', '$7.99', '$4.95'],
            ['China (CNY)', 'CNY 33.00', 'CNY 20.46'],
            ['Spain (EUR)', 'EUR 7.49', 'EUR 4.64'],
            ['Japan (JPY)', 'JPY 880', 'JPY 545'],
            ['South Korea (KRW)', 'KRW 8,400', 'KRW 5,210'],
            ['Germany (EUR)', 'EUR 7.49', 'EUR 4.64'],
            ['France (EUR)', 'EUR 7.49', 'EUR 4.64'],
            ['Brazil (BRL)', 'BRL 23.99', 'BRL 14.87'],
            ['Italy (EUR)', 'EUR 7.49', 'EUR 4.64'],
          ],
        },
        afterTable: 'Steam returned these values for the listed storefront country codes on the check date. The table is not a forecast, a historical price chart, or a guarantee that changing account region is allowed; keep your Steam account region aligned with your real place of residence and Steam’s current rules.',
      },
      {
        id: 'list-price-and-sale',
        title: 'List price versus the temporary Steam sale price',
        paragraphs: [
          'The list price is the reference amount shown when no discount applies. A sale price is a time-limited offer when Steam displays one. In the September 25 snapshot, the sale amount was 38% lower across the nine storefronts checked. The API response did not include the promotion’s end time, so this guide cannot verify how long the offer lasted or whether it remains active after the check.',
          'Searches such as “how much is PEAK” can mix these two answers. Use the list-price column to understand the normal storefront amount in this snapshot and the sale-price column only as a record of the dated offer. Before paying, refresh the regional store page and compare the price in your cart. A discount badge or amount copied from an older post is not evidence of today’s price.',
        ],
        bullets: [
          ['List price', 'The regular amount returned for the storefront at the time of the API check.'],
          ['Sale price', 'The discounted amount observed on September 25, 2026; the end date was not returned.'],
          ['Checkout amount', 'The current figure in your own Steam cart, including any applicable local charges.'],
        ],
      },
      {
        id: 'what-the-price-covers',
        title: 'What you are buying when you pay for PEAK',
        paragraphs: [
          'PEAK is a cooperative climbing and survival game sold through its Steam product page. Its core activity is planning a route, climbing through changing terrain, managing limited resources, and helping other scouts recover from mistakes. The gameplay image below shows the product context; it does not document store features, price, or a particular edition.',
          'This guide compares the standard PEAK Steam listing only. It does not claim that optional content, a bundle, a soundtrack, or a future edition is included unless the live product page says so. Check the current listing’s purchase section before buying, especially if the store shows more than one package. The game’s actual content and platform availability are separate questions from its regional price.',
          'For a plain-language overview of the co-op loop, maps, and supported play context, start with our [[link:what-is-peak-game|guide to what PEAK is]]. That context can help you judge whether the game fits your group, while the official store remains the source for the item you are purchasing.',
        ],
        image: {
          src: '/media/peak-coop-climb.webp',
          width: 1200,
          height: 675,
          alt: 'PEAK scouts climbing together toward a mountain summit',
          caption: 'Official Steam gameplay screenshot: a view of PEAK’s co-op climbing context, not a price screen.',
        },
      },
      {
        id: 'check-live-steam-price',
        title: 'How to check the current PEAK Steam price',
        paragraphs: [
          'To answer “how much does PEAK cost on Steam?” for your own account, use the regional product page rather than relying on a search snippet or a dated comparison. Check the currency and discount label on the page, then confirm the same amount in the cart immediately before you pay. A different language setting alone does not prove that the storefront region has changed.',
          'A sensible check takes less than a minute and avoids most stale-price confusion. If a friend quotes a different amount, compare the country and currency on each store page first. Keep the disagreement factual: the figures can both be accurate for different regions or dates. Do not change your account country merely to reproduce another row in this guide.',
        ],
        bullets: [
          ['1. Open the official listing', 'Use the PEAK product page linked in Sources and verify the signed-in store country.'],
          ['2. Read the currency and offer', 'Separate the list price from any visible discount and note the date.'],
          ['3. Check the cart', 'Confirm the live total and any applicable local charges before completing the purchase.'],
          ['4. Recheck platform fit', 'This page covers Steam pricing; see our [[link:room-codes|platform and multiplayer guide]] for a separate compatibility overview.'],
        ],
        image: {
          src: '/media/peak-map-route.webp',
          width: 1200,
          height: 675,
          alt: 'PEAK scout using a rope beside a lava hazard during a climb',
          caption: 'Official Steam gameplay screenshot: inspect the game context separately from the storefront price.',
        },
      },
      {
        id: 'price-history-and-buying-decision',
        title: 'Can this page predict the next PEAK discount?',
        paragraphs: [
          'No. A single dated observation cannot establish a sale calendar, lowest-ever price, or likely next discount. Steam’s product page and official announcements are the dependable places to check an active offer. This page intentionally avoids a price-history graph because the sources collected for this brief did not provide a complete, verifiable historical series for PEAK.',
          'If you are deciding whether to buy now, compare the current amount with the budget you set for the game and the number of people who will join you. Wait only if that suits your own budget; do not treat the September snapshot as a recommendation that a sale will return. For platform availability and multiplayer questions, use the separate [[link:room-codes|PEAK platform and room-code guide]] rather than inferring support from a price table.',
          'The distinction between verified fact and buying advice matters: the amounts above came from Steam’s official storefront data on one date; whether that amount is good value is a personal judgment based on your group and preferences. Revisit the live listing when you are ready to purchase.',
        ],
      },
    ],
    faq: {
      eyebrow: 'Quick answers',
      title: 'PEAK game price FAQ',
      items: [
        ['How much is PEAK in the US?', 'Steam’s official data showed a $7.99 list price and a $4.95 sale price for the US storefront on September 25, 2026. The discount was a dated snapshot; check the current listing and cart before buying.'],
        ['How much does PEAK cost on Steam?', 'The amount depends on the Steam storefront region and whether a current offer is active. The table records nine official regional prices checked on September 25, 2026; your live regional product page is authoritative for today.'],
        ['Is the PEAK sale price permanent?', 'No. The $4.95 US price and other discounted amounts were observed in one snapshot. Steam’s response did not provide an offer end date, so this page does not claim the sale is still active.'],
        ['Why does PEAK have different prices by country?', 'Steam storefronts can show regional currencies and region-specific pricing. This comparison reports what the official data returned; it does not explain or predict the publisher’s pricing decisions.'],
        ['Does this PEAK price guide cover PlayStation or Xbox?', 'No. It compares Steam storefronts only. For a separate platform and multiplayer overview, read the [[link:room-codes|PEAK platform guide]] and confirm current availability with the relevant official store.'],
        ['Does PEAK have a verified historical lowest price?', 'This page does not establish one. The research for this guide did not find a complete first-party price-history series, so it avoids calling any amount the all-time low.'],
      ],
    },
    source: {
      eyebrow: 'Sources and limits',
      title: 'Official Steam data, checked on a fixed date',
      body: 'Regional list and sale amounts were read from Steam’s official app-details storefront response for the nine listed country codes on September 25, 2026. Steam’s public PEAK listing is linked below. The response recorded the observed discount but no offer end date; use the live page and cart for a current purchase decision. This guide is an independent fan reference and does not sell the game.',
      links: [
        ['Official PEAK Steam listing', 'https://store.steampowered.com/app/3527290/PEAK/'],
        ['Official Steam app-details data (US storefront)', 'https://store.steampowered.com/api/appdetails?appids=3527290&cc=us&l=en'],
      ],
    },
    related: [
      ['what-is-peak-game', 'Read what PEAK is'],
      ['room-codes', 'Check platforms and multiplayer'],
      ['peak-game-update', 'Review official PEAK updates'],
    ],
    relatedLabel: 'Related PEAK guides',
  },
};
